  ...
  <div class="content">
    ...
    <div class="my-table">
      <el-table :data="list" ref="table" style="width: 100%" height="100%" :stripe="true" highlight-current-row size="mini">
        <el-table-column prop="bottomCommissionRecommendId" label="ID"></el-table-column>
        ...
        <el-table-column prop="mainPic" label="主图">
          <template slot-scope="scope">
            <img style="width:110px" @click="prev(scope.$index)" :src="scope.row.mainPic" alt="">
          </template>
        </el-table-column>
    ...
    <v-dialog :width="400" :zIndex="10" :dialogShow="dialogFormVisible" :title="isedit?'编辑底部返佣推荐商品':'新增底部返佣推荐商品'">
...
import { bottomCommissionRecommendList, addBottomCommissionRecommend, bottomCommissionRecommendUp, commissionGoodsList } from '@/api'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    edit (row) {
      this.form.bottomCommissionRecommendId = row.bottomCommissionRecommendId
    ...
    del (row) {
      this.$confirm('此操作将删除底部返佣推荐商品，是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        const res = await bottomCommissionRecommendUp({ bottomCommissionRecommendIds: row.bottomCommissionRecommendId, status: 3 })
        if (res.result === '1') {
          this.getList()
        }
      }).catch(() => {
    ...
    async getList () {
      const res = await bottomCommissionRecommendList(this.requestData)
      if (res.result === '0') {
    ...
    async create () {
      const res = await addBottomCommissionRecommend(this.form)
      if (res.result === '1') {
    ...
    freeze (row) {
      let tip
      row.status === 2 ? tip = '此操作将启用该底部返佣推荐商品，是否继续？' : tip = '此操作将禁用该底部返佣推荐商品，是否继续？'
      this.$confirm(tip, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this._freeze(row.status === 2 ? 1 : 2, row.bottomCommissionRecommendId)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '操作取消'
    ...
    async _freeze (status, bottomCommissionRecommendId) {
      const res = await bottomCommissionRecommendUp({ status: status, bottomCommissionRecommendIds: bottomCommissionRecommendId })
      if (res.result === '1') {
        this.getList()
      }
    },
    ...
    async getQueryList () {
      const res = await commissionGoodsList(this.requestData1)
      if (res.result === '0') {
