  ...
  <div class="content">
    ...
    <div class="my-table">
      <el-table :data="list" ref="table" style="width: 100%" height="100%" :stripe="true" highlight-current-row size="mini">
        <el-table-column prop="advertId" label="轮播图ID"></el-table-column>
        ...
        <el-table-column prop="advertPic" width="130" label="轮播图">
          <template slot-scope="scope">
            <img style="width:110px" @click="prev(scope.$index)" :src="baseUrl + scope.row.advertPic" alt="">
          </template>
        </el-table-column>
    ...
    <v-dialog :width="400" :zIndex="10" :dialogShow="dialogFormVisible" :title="isedit?'编辑轮播图':'新增轮播图'">
      ...
      <div slot="con">
        <el-form ref="form" @submit.native.prevent :model="form" label-width="90px">
          ...
          <el-form-item label="轮播图" prop="advertPic">
            <span style="position:absolute;left:-65px;color:#eb453c;">*</span>
...
import { advertList, addAdvert, advertUp, commissionGoodsList, takeDeliveryGoodsList } from '@/api'
export default {
  data () {
    return {
      ...
      list: [],
      total: 0,
      form: { goodsId: '', advertPic: '', sort: '', type: '' },
      submitTitle: '确定',
      subLoad: false,
      baseUrl: baseUrl,
      isedit: false,
  ...
  computed: {
    cansubmit: function () {
      return (this.form.goodsId === '' || this.form.advertPic === '' || this.form.sort === '' || this.form.type === '' || this.subLoad)
    }
  },
  methods: {
    ...
    upfiles (file) {
      this.form.advertPic = file.files[0]
    },
    updelete (file) {
      this.form.advertPic = ''
    },
    ...
    edit (row) {
      this.form.advertId = row.advertId
      this.goodsName = row.businessTitle
      this.form.goodsId = row.goodsId
      this.form.sort = row.sort
      this.form.type = row.type
      this.form.advertPic = row.advertPic
      this.option.file = [row.advertPic]
      this.dialogFormVisible = true
      this.isedit = true
    },
    del (row) {
      this.$confirm('此操作将删除轮播图，是否继续？', '提示', {
        ...
        const res = await advertUp({ advertIds: row.advertId, status: 3 })
        if (res.result === '1') {
          this.getList()
        }
      }).catch(() => {
    ...
    async getList () {
      const res = await advertList(this.requestData)
      if (res.result === '0') {
    ...
    setReset () {
      this.option.file = []
      this.form = { goodsId: '', advertPic: '', sort: '', type: '' }
    },
    submit () {
    ...
    async create () {
      const res = await addAdvert(this.form)
      if (res.result === '1') {
    ...
    freeze (row) {
      let tip
      row.status === 2 ? tip = '此操作将启用该轮播图，是否继续？' : tip = '此操作将禁用该轮播图，是否继续？'
      this.$confirm(tip, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this._freeze(row.status === 2 ? 1 : 2, row.advertId)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '操作取消'
    ...
    async _freeze (status, advertId) {
      const res = await advertUp({ status: status, advertIds: advertId })
      if (res.result === '1') {
        this.getList()
      }
    },
    prev (index) {
      const srcs = this.list.map(item => {
        return this.baseUrl + item.advertPic
      })
