  ...
  <div class="content">
    ...
    <div class="my-table">
      <el-table :data="list" ref="table" style="width: 100%" height="100%" :stripe="true" highlight-current-row size="mini">
        <el-table-column prop="commissionRecommendId" label="ID"></el-table-column>
    ...
    <v-dialog :width="400" :zIndex="10" :dialogShow="dialogFormVisible" :title="isedit?'编辑返佣推荐商品':'新增返佣推荐商品'">
...
import { commissionRecommendList, addCommissionRecommend, commissionRecommendUp, commissionGoodsList } from '@/api'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    edit (row) {
      this.form.commissionRecommendId = row.commissionRecommendId
    ...
    del (row) {
      this.$confirm('此操作将删除返佣推荐商品，是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        const res = await commissionRecommendUp({ commissionRecommendIds: row.commissionRecommendId, status: 3 })
        if (res.result === '1') {
          this.getList()
        }
      }).catch(() => {
    ...
    async getList () {
      const res = await commissionRecommendList(this.requestData)
      if (res.result === '0') {
    ...
    async create () {
      const res = await addCommissionRecommend(this.form)
      if (res.result === '1') {
    ...
    freeze (row) {
      let tip
      row.status === 2 ? tip = '此操作将启用该返佣推荐商品，是否继续？' : tip = '此操作将禁用该返佣推荐商品，是否继续？'
      this.$confirm(tip, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this._freeze(row.status === 2 ? 1 : 2, row.commissionRecommendId)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '操作取消'
    ...
    async _freeze (status, commissionRecommendId) {
      const res = await commissionRecommendUp({ status: status, commissionRecommendIds: commissionRecommendId })
      if (res.result === '1') {
        this.getList()
      }
    },
